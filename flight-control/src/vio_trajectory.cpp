...
#endif
void planning(const nav_msgs::Odometry::ConstPtr& msg)
{
    traj = *msg;
}
...
void follow(vir& vir, geometry_msgs::PoseStamped& host_mocap, geometry_msgs::TwistStamped* vs,float vx,float vy,float yaw_dot)
{
    ...
    if(uroll<=-0.4 ||uroll>=0.4)
    {
    uroll = 0.4*uroll/abs(uroll);
    }
...
#endif
    ros::Subscriber traj_sub = nh.subscribe<nav_msgs::Odometry> ("/result",2, planning);
...
//        ROS_WARN("extrinsic: %.2f, %.2f, %.2f", cam2imu.pose.pose.position.x, cam2imu.pose.pose.position.y, cam2imu.pose.pose.position.z);
    ...
    while (ros::ok()) {
        ...
        if (start == true) {
          ...
          desired_pos.x = traj.pose.pose.position.x;
          desired_pos.y = traj.pose.pose.position.y;
          desired_pos.z = traj.pose.pose.position.z;

          desired_vel.twist.linear.x = traj.twist.twist.linear.x;
          desired_vel.twist.linear.y = traj.twist.twist.linear.y;
          desired_vel.twist.linear.z = 0;

          vir1.x = initial_pose.pose.position.x + desired_pos.x;
          vir1.y = initial_pose.pose.position.y + desired_pos.y;
          yaw_angle = traj.pose.pose.orientation.z;
          vir1.roll = yaw_angle;
          yaw_dot = traj.pose.pose.orientation.w;
#else
